<form wire:submit.prevent="submit" class="pt-3">
    ...
    <div class="form-group {{ $errors->has('user.songs') ? 'invalid' : '' }}">
        <label class="form-label" for="songs">{{ trans('cruds.user.fields.songs') }}</label>
        <input class="form-control" type="text" name="songs" id="songs" wire:model.defer="user.songs">
        <div class="validation-message">
            {{ $errors->first('user.songs') }}
        </div>
        <div class="help-block">
            {{ trans('cruds.user.fields.songs_helper') }}
        </div>
    </div>
